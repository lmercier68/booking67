"use strict";Object.defineProperty(exports, "__esModule", {value: true});"use client";


var _GXNCC5AVcjs = require('../__chunks/GXNCC5AV.cjs');


var _YRDDHZT7cjs = require('../__chunks/YRDDHZT7.cjs');
require('../__chunks/RJZ35XFC.cjs');
require('../__chunks/KKBPJ4X4.cjs');
require('../__chunks/XCM6Y3AX.cjs');
require('../__chunks/F7HB6RYG.cjs');
require('../__chunks/FQGSRSYD.cjs');
require('../__chunks/JVORNAMD.cjs');
require('../__chunks/ORT5QOHH.cjs');
require('../__chunks/KZA6P235.cjs');
require('../__chunks/QKKT6RXX.cjs');
require('../__chunks/SMKW5GKS.cjs');
require('../__chunks/7ARL6QHN.cjs');
require('../__chunks/75KXQZJX.cjs');




var _VKYTJKYNcjs = require('../__chunks/VKYTJKYN.cjs');
require('../__chunks/JTVNRMAC.cjs');
require('../__chunks/LNE2FWJO.cjs');



var _WYXAWBLEcjs = require('../__chunks/WYXAWBLE.cjs');

// src/menu/menu-dismiss.ts
var useMenuDismiss = _VKYTJKYNcjs.createHook.call(void 0, 
  (_a) => {
    var _b = _a, { store } = _b, props = _WYXAWBLEcjs.__objRest.call(void 0, _b, ["store"]);
    const context = _YRDDHZT7cjs.useMenuScopedContext.call(void 0, );
    store = store || context;
    props = _GXNCC5AVcjs.useHovercardDismiss.call(void 0, _WYXAWBLEcjs.__spreadValues.call(void 0, { store }, props));
    return props;
  }
);
var MenuDismiss = _VKYTJKYNcjs.createComponent.call(void 0, (props) => {
  const htmlProps = useMenuDismiss(props);
  return _VKYTJKYNcjs.createElement.call(void 0, "button", htmlProps);
});
if (process.env.NODE_ENV !== "production") {
  MenuDismiss.displayName = "MenuDismiss";
}



exports.MenuDismiss = MenuDismiss; exports.useMenuDismiss = useMenuDismiss;
