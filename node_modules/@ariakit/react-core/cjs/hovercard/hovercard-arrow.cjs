"use strict";Object.defineProperty(exports, "__esModule", {value: true});"use client";


var _ZDTJSOSWcjs = require('../__chunks/ZDTJSOSW.cjs');
require('../__chunks/AH3CUB6N.cjs');


var _ORT5QOHHcjs = require('../__chunks/ORT5QOHH.cjs');
require('../__chunks/KZA6P235.cjs');
require('../__chunks/QKKT6RXX.cjs');
require('../__chunks/SMKW5GKS.cjs');




var _VKYTJKYNcjs = require('../__chunks/VKYTJKYN.cjs');
require('../__chunks/JTVNRMAC.cjs');
require('../__chunks/LNE2FWJO.cjs');



var _WYXAWBLEcjs = require('../__chunks/WYXAWBLE.cjs');

// src/hovercard/hovercard-arrow.ts
var useHovercardArrow = _VKYTJKYNcjs.createHook.call(void 0, 
  (_a) => {
    var _b = _a, { store } = _b, props = _WYXAWBLEcjs.__objRest.call(void 0, _b, ["store"]);
    const context = _ORT5QOHHcjs.useHovercardContext.call(void 0, );
    store = store || context;
    props = _ZDTJSOSWcjs.usePopoverArrow.call(void 0, _WYXAWBLEcjs.__spreadValues.call(void 0, { store }, props));
    return props;
  }
);
var HovercardArrow = _VKYTJKYNcjs.createComponent.call(void 0, 
  (props) => {
    const htmlProps = useHovercardArrow(props);
    return _VKYTJKYNcjs.createElement.call(void 0, "div", htmlProps);
  }
);
if (process.env.NODE_ENV !== "production") {
  HovercardArrow.displayName = "HovercardArrow";
}



exports.HovercardArrow = HovercardArrow; exports.useHovercardArrow = useHovercardArrow;
